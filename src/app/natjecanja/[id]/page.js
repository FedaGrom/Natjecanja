...
import { doc, updateDoc, deleteField, getDoc, query, collection, where, onSnapshot, arrayUnion, arrayRemove, serverTimestamp } from "firebase/firestore";
...
export default function DetaljiNatjecanja() {
  ...
  // Add missing gallery viewer state
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      const localNatjecanja = JSON.parse(localStorage.getItem('natjecanja') || '[]');
      const updatedNatjecanja = localNatjecna.map(n => 
  ...
  const handleAddGalleryImageUrl = async () => {
    ...
    try {
      const ref = doc(db, 'natjecanja', natjecanje.id);
      await updateDoc(ref, { slike: arrayUnion(url) });
      setNatjecanje(prev => ({ ...prev, slike: [...(prev?.slike || []), url] }));
      await Swal.fire('Dodano', 'Slika je dodana u galeriju.', 'success');
    } catch (e) {
      console.error('Error adding gallery image:', e);
      await Swal.fire('Greška', 'Nije moguće dodati sliku.', 'error');
    }
  };
  ...
  const handleRemoveGalleryImageUrl = async (url) => {
    ...
    try {
      const ref = doc(db, 'natjecanja', natjecanje.id);
      await updateDoc(ref, { slike: arrayRemove(url) });
      setNatjecanje(prev => ({ ...prev, slike: (prev?.slike || []).filter(u => u !== url) }));
      await Swal.fire('Uklonjeno', 'Slika je uklonjena iz galerije.', 'success');
    } catch (e) {
      console.error('Error removing gallery image:', e);
      await Swal.fire('Greška', 'Nije moguće ukloniti sliku.', 'error');
    }
  };

  const openGalleryAt = (i) => {
    if (!Array.isArray(natjecanje?.slike) || natjecanje.slike.length === 0) return;
    setGalleryIndex(i);
    setIsGalleryOpen(true);
  };
  const closeGallery = () => setIsGalleryOpen(false);
  const nextImage = () => {
    if (!natjecanje?.slike?.length) return;
    setGalleryIndex((i) => (i + 1) % natjecanje.slike.length);
  };
  const prevImage = () => {
    if (!natjecanje?.slike?.length) return;
    setGalleryIndex((i) => (i - 1 + natjecanje.slike.length) % natjecanje.slike.length);
  };
  ...
  }, [isGalleryOpen, natjecanje?.slike]);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Header */}
      <header className="sticky top-0 inset-x-0 w-full bg-[#666] shadow-md border-b border-gray-200 z-[100]">
      ...
      <div className="overflow-x-hidden">
        {/* KONTEJNER: zadrži prostor za sidebar, padding seli u unutarnji wrapper */}
        <div className="max-w-none mx-0 lg:ml-80">
          {/* centrirani unutarnji wrapper: šire na lg/xl */}
          <div className="mx-auto w-full max-w-[1400px] xl:max-w-[1500px] px-4 lg:px-8">
            {/* grid stupci: blago širi lijevi stupac */}
            <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-6 items-start">
              {/* Lijevi stupac (ostaje isti) */}
                          ...
                          <div className="flex items-center gap-4 mt-2">
                            <label className="inline-flex items-center gap-2 text-sm">
                              <input
                                ...
                                onChange={handleFormChange}
                              />
                              <span className="text-black">Na webu</span>
                            </label>
                            <label className="inline-flex items-center gap-2 text-sm">
                              <input
                                type="radio"
                                ...
                                onChange={handleFormChange}
                              />
                              <span className="text-black">Vanjski link</span>
                            </label>
                          </div>
                        </div>
              ...
              {/* Right: applications list + Gallery */}
              <aside className="lg:col-span-1">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden h-full flex flex-col">
                  ...
                  {/* Gallery section (visible; controls only in edit mode) */}
                  <div className="border-t p-4">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-lg font-bold text-gray-800">Slike</h3>
                      {canEdit && editMode && (
                        <button
                    ...
                    {(natjecanje?.slike && natjecanje.slike.length > 0) ? (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {natjecanje.slike.map((url, i) => (
                          <div key={url} className="relative group">
                            ...
                            {canEdit && editMode && (
                              <button
                                onClick={() => handleRemoveGalleryImageUrl(url)}
                                className="absolute top-2 right-2 bg-gray-700/80 text-white rounded px-2 py-1 text-xs opacity-0 group-hover:opacity-100 transition-opacity"
                                title="Ukloni"
                                type="button"
                              >
                                Ukloni
      ...
      {isGalleryOpen && Array.isArray(natjecanje?.slike) && natjecanje.slike.length > 0 && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center">
          <button
            onClick={closeGallery}
            className="absolute top-4 right-4 bg-white/90 text-black px-3 py-2 rounded shadow hover:bg-white"
            title="Zatvori"
          >
            Zatvori
          </button>
          <button
            onClick={prevImage}
            className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/90 text-black px-3 py-2 rounded shadow hover:bg-white"
            title="Prethodna"
          >
            ‹
          </button>
          <img
            src={natjecanje.slike[galleryIndex]}
            alt={`Slika ${galleryIndex + 1}`}
            className="max-w-[90vw] max-h-[80vh] object-contain rounded shadow-lg"
          />
          <button
            onClick={nextImage}
            className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/90 text-black px-3 py-2 rounded shadow hover:bg-white"
            title="Sljedeća"
          >
            ›
          </button>
